...
# pdf(file = "LogFoldCounts_sum_dat_all_long.pdf",   
#     width = 6,
#     height = 4.6) 

Figure_2A <- ggballoonplot(
  LogFoldCounts_sum_dat_all_long_ALLComp, x = "var", y="Day",
...
# pdf(file = "df_fig_day4_ANCOMBC_Final.pdf",   # The directory you want to save the file in
#     width = 8.3, # The width of the plot in inches
#     height = 9.7) # The height of the plot in inches

Figure_2B <- df_fig_day4_ANCOMBC_Final %>% filter(LogFoldChange!=0) %>%
  ...
  facet_grid(vars(Env), vars(Comparison), scales = "free_y") + 
  theme_bw() +
  theme(#axis.line = element_line(color='black'),
    plot.background = element_blank(),
...
pdf(file = "Figures/Figure_2.pdf",   # The directory you want to save the file in
...
ggdraw(Figure_2B_1) + draw_label("B", x = 0.5, y = 0.97, fontface = "bold") + draw_label("A", x = 0.025, y = 0.97, fontface = "bold") +
  draw_plot(Figure_2A + theme(legend.position = "bottom", text = element_text(size=10)), 
            x = 0.45, y = 0.97, hjust = 1, vjust = 1, width = 0.45, height = 0.55)


dev.off()


tiff("Figures/Figure_2.tiff", units="in", width=8.5, height=9, res=300)
